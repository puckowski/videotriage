package com.keypointforensics.videotriage.filter.blocking;

import com.keypointforensics.videotriage.filter.errorlevel.Util;
...
public class BlockingExtractor {
	...
	private void getBLKArtifact(String fileName) throws IOException {
		...
		for (int ii = 0; ii < y.length; ii++) {
			for (int jj = 0; jj < y[0].length; jj++) {
				...
				y[ii][jj] = (int) Math.round((65.481 * ((float) origByteImage[0][ii][jj] / 255)
						+ 128.553 * ((float) origByteImage[1][ii][jj] / 255)
						+ 24.966 * ((float) origByteImage[2][ii][jj]) / 255));
			}
		}
	...
	private float[][] blockProcess(int[][] ImIn) {
		float[][] outArray = new float[(int) Math.ceil((ImIn.length - 1) / 8)][(int) Math
				.ceil((ImIn[0].length - 1) / 8)];
	...
	private int BlockFun(int[][] block) {
		...
		if (rowEdge[0] < rowEdge[1])
			min1 = rowEdge[0];
		else
			min1 = rowEdge[1];
		if (colEdge[0] < colEdge[1])
			min2 = colEdge[0];
		else
			min2 = colEdge[1];
