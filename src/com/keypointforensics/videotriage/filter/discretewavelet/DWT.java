...
public class DWT {
	 ...
	 */
	public static double[] transform(double[] signal, Wavelet wavelet, int order, int L, Direction direction)
			throws Exception {
	 ...
	 *             Invalid wavelet parameters
	 */
	public static double[] forwardDwt(double[] signal, Wavelet wavelet, int order, int L) throws Exception {

		long begin, timer1 = 0, timer2 = 0, timer3 = 0, timer4 = 0, timer5 = 0, timer6 = 0;
		int n = signal.length;
		if (!isValidChoices(wavelet, order, L, n)) {
			throw new Exception("Invalid wavelet /order/scale/signal-length combination.");
		}
		...
		// original code replaced with ojalgo implementation -up to 4-fold speed
		// increase
	 ...
	 *             a
	 */
	public static double[] inverseDwt(double[] signal, Wavelet wavelet, int order, int L) throws Exception {
		int n = signal.length;
		if (!isValidChoices(wavelet, order, L, n)) {
			throw new Exception("Invalid wavelet /order/scale/signal-length combination.");
		}
	...
	/**
	 * Multi-resolution analysis See: Mallet, A Wavelet Tour of Signal Processing,
	 * the Sparse Way, 2008, pp. 170-172
	 *
	 * @param signal
	 *            a double[] with length = even power of two
	 * @param wavelet
	 ...
	 *         finest scale to the coarsest scale terminating with the approximation
	 *         curve; and result.get(1) is an array of j values corresponding to the
	 *         scales used to create the data where: scale = 2^-j and the
	 *         terminating value is set to "0" for the approximation scale.
	 * @throws Exception
	 *             Invalid wavelet parameters
	 */
	public static ArrayList<Object> mRA(double[] signal, Wavelet wavelet, int order, int L) throws Exception {
		...
		for (int j = (J - 1); j >= L; j--) {
			int[] dyad = DWT.dyad(j);
			scalesUsed[scaleCounter] = (int) (Math.log(dyad.length) / Math.log(2));
			scaleCounter++;
		}
	 ...
	 * Then result = {5, 6, 3, 4}
	 *
	 * @return the first count elements in source overwrite the first count elements
	 *         in the result.
	 */
	private static double[] subCopy(double[] source, double[] destination, int count) {
	 ...
	 * @param x
	 *            the sequence to pad
	 * @return If necessary, expanded sequence such that its length is an even power
	 *         of 2 by adding additional zero values.
	 */
	 ...
	 * @param signalLength
	 *            To be explained
	 * @return true only if the wavelet/order exists in OrthogonalFileters.java and
	 *         the signal is long enough to be analyzed at the chosen coarsest scale
	 *         L.
	 */
	private static boolean isValidChoices(Wavelet wavelet, int order, int L, int signalLength) {
		try {
			ArrayList<Integer> validParams = OrthogonalFilters.validParameters(wavelet);
			boolean isValid = false;
			for (int i = 0; i < validParams.size(); i++) {
				if (validParams.get(i) == Integer.valueOf(order)) {
					ArrayList<Integer> validScales;
					validScales = OrthogonalFilters.validScales(order, wavelet, signalLength);
	 ...
	 * using each of the orthogonal filters provided in OrthogonalFilters.java for
	 * all valid orders provided.
	 *
	 * @throws Exception
	...
	public static void relativeErrors() throws Exception {
		...
		double[][] testFile = MatrixOps.transpose((double[][]) FileOps.openMatrix(file));
		...
		for (Wavelet wavelet : Wavelet.values()) {
			ArrayList<Integer> params = OrthogonalFilters.validParameters(wavelet);
			for (Integer param : params) {
				Integer coarsest = OrthogonalFilters.validScales(param, wavelet, fnLen).get(0);
				double[] dwt = transform(testFn, wavelet, param, coarsest, Direction.forward);
				idwt = transform(dwt, wavelet, param, coarsest, Direction.reverse);
				double resid = MatrixOps.vector2Norm(MatrixOps.add(testFn, MatrixOps.scale(-1.0, idwt)));
