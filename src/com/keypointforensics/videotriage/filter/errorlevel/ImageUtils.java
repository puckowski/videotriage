...
public class ImageUtils {
	 ...
	 * 
	 * @param img
	 *            BufferedImage, the input image from which to extract RGB
	 ...
	 * 
	 * @param raw
	 *            int[][][] representing RGB pixels of image.
	...
	public static BufferedImage RGBImg(int[][][] raw) {
		...
		if (height > 0 && width > 0 || raw[0][0].length == 3) {
			...
			for (int row = 0; row < height; row++) {
				for (int column = 0; column < width; column++) {
					img.setRGB(column, row,
							(raw[row][column][0] << 16) | (raw[row][column][1] << 8) | (raw[row][column][2]));
				}
			}
		}
	 ...
	 * 
	 * @param bits
	 *            The 32-bit BufferedImage pixel value
	 * @return RGB values extracted from pixel
	 */
	private static int[] intRGB(int bits) {
		// Java rgb values are actually 4 bytes (r,g,b,a) pressed into one 32-bit
		// integer
	...
	/**
	 * Send this method a BufferedImage base, a BufferedImage mask, an int[] mask
	 * color, and an int threshold to mask the base image with all pixels in the
	 * mask image that meet/exceed the threshold with the supplied color.
	 * 
	 * @param baseImage
	 *            The base image which is to be masked over.
	 * @param maskImage
	 *            The masking image. This is a difference image.
	 * @param maskColor
	 *            The RGB pixel values desired for the mask color.
	 * @param threshold
	 *            Max pixel value (r+g+b) allowed before marking pixel as changed.
	 * @return BufferedImage where the base image has 'changed' pixels masked.
	 */
	public static BufferedImage MaskImages(BufferedImage baseImage, BufferedImage maskImage, int[] maskColor,
			int threshold) {
