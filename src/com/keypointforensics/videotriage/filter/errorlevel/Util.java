...
public class Util {
	...
	public static BufferedImage recompressImage(BufferedImage imageIn, int quality) {
		// Apply in-memory JPEG compression to a BufferedImage given a quality setting
		// (0-100)
		...
		for (int ii = 0; ii < newHeight; ii++) {
			for (int jj = 0; jj < newWidth; jj++) {
				try {
					...
					System.out.println(newHeight + " " + newWidth + " " + image1.getHeight() + " " + image1.getWidth()
							+ " " + ii + " " + jj + " " + Math.round(ii * heightModifier) + " "
	...
	public static double minDouble3DArrayD(double[][][] arrayIn) {
		...
		for (double[][] twoDInRow : arrayIn) {
			colMin = Arrays.stream(twoDInRow).flatMapToDouble(a -> Arrays.stream(a)).min().getAsDouble();
			if (colMin < min) {
				min = colMin;
			}
		}
	...
	/*
	 * public static double maxDouble3DArray(float[][][] arrayIn) { // Calculate the
	 * maximum value of a 3D float array double max = -Double.MAX_VALUE; double
	 * colMax; for (float[][] twoDInRow : arrayIn) { for (float[] arrayInRow :
	 * twoDInRow) { List b = Arrays.asList(ArrayUtils.toObject(arrayInRow)); colMax
	 * = (float) Collections.max(b); if (colMax > max) { max = colMax; } } } return
	 * max; }
	 */
	...
	public static double maxDouble3DArrayD(double[][][] arrayIn) {
		...
		double colMax;
		for (double[][] twoDInRow : arrayIn) {
			colMax = Arrays.stream(twoDInRow).flatMapToDouble(a -> Arrays.stream(a)).max().getAsDouble();
			if (colMax > max) {
				max = colMax;
			}
		}
		...
		for (int ii = 0; ii < newWidth; ii++) {
			for (int jj = 0; jj < newHeight; jj++) {
				try {
					...
					System.out.println(outputMap.length + " " + outputMap[0].length + " " + origMap.length + " "
							+ origMap[0].length + " " + ii + " " + jj + " " + Math.round(ii * heightModifier) + " "
							+ Math.round(jj * widthModifier) + " " + heightModifier + " " + widthModifier);
				}
			}
		}
