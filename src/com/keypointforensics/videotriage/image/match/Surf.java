...
public class Surf implements Serializable {
	...
	private void getOrientation(SURFInterestPoint input) {
		...
		// System.out.println("s = " + s + ", r = " + r + ", c = " + c + ", scale = " +
		// scale);
		...
		for (int i = -6; i <= 6; ++i) {
			for (int j = -6; j <= 6; ++j) {
				if (i * i + j * j < 36) {
					...
					// System.out.format("gauss = %.8f, haarX = %.8f, haarY = %.8f, getAngle(x,y) =
					// %.8f",gauss,xHaarResponse,yHaarResponse,getAngle(xHaarResponse,yHaarResponse));
					// System.out.println();
				}
			}
		}
