...
public class FastHessian implements Serializable {
	...
	private static int[][] filter_map = { { 0, 1, 2, 3 }, { 1, 3, 4, 5 }, { 3, 5, 6, 7 }, { 5, 7, 8, 9 },
			{ 7, 9, 10, 11 } };
	...
	private SURFInterestPoint interpolateExtremum(int r, int c, ResponseLayer t, ResponseLayer m, ResponseLayer b) {
		// should check to make sure that m's filter value is less than t's and greater
		// than b's
		...
		if (Math.abs(xi) < 0.5f && Math.abs(xr) < 0.5f && Math.abs(xc) < 0.5f) {
			...
			// System.out.println("x: " + x + ", y: " + y + ", scale: " + scale + ", lap: "
			// + laplacian);
	...
	private RealMatrix getPartialDerivativeMatrix(int r, int c, ResponseLayer t, ResponseLayer m, ResponseLayer b) {
		...
		// System.out.format("dx = %.8f, dy = %.8f, ds =
		// %.8f",derivs[0][0],derivs[1][0],derivs[2][0]);
		// System.out.println();

		RealMatrix matrix = new Array2DRowRealMatrix(derivs);
		// System.out.println("Matrix Num Rows: " + matrix.getRowDimension() + ", num
	...
	private RealMatrix getHessian3DMatrix(int r, int c, ResponseLayer t, ResponseLayer m, ResponseLayer b) {
		...
		// dxy
		hessian[0][1] = hessian[1][0] = (m.getResponse(r + 1, c + 1, t) - m.getResponse(r + 1, c - 1, t)
				- m.getResponse(r - 1, c + 1, t) + m.getResponse(r - 1, c - 1, t)) / 4.0;

		// dxs
		hessian[0][2] = hessian[2][0] = (t.getResponse(r, c + 1) - t.getResponse(r, c - 1) - b.getResponse(r, c + 1, t)
				+ b.getResponse(r, c - 1, t)) / 4.0;

		// dys
		hessian[1][2] = hessian[2][1] = (t.getResponse(r + 1, c) - t.getResponse(r - 1, c) - b.getResponse(r + 1, c, t)
	...
	public static void main(String[] args) {
		try {
			IntegralImage integralImage = new IntegralImage(
					ImageIO.read(new File("C:\\workspace\\opensurf\\OpenSURF\\Images\\sf.jpg")));
