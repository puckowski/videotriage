...
public class BlobDetectionThread extends Thread {
	...
	private final String                   VIDEO_CREATION_DATE;
	private final int                      VIDEO_CREATION_DATE_MODE;
	...
	public BlobDetectionThread(final String controllerId, final BufferedImage backgroundImage, 
			...
			final String videoCreationDate) {	
		...
		VIDEO_CREATION_DATE_MODE    = CONTROLLER_REGISTRY.getMetadataParams(CONTROLLER_ID).getDateCreationMode();
		...
		while (Thread.currentThread().isInterrupted() == false) {
			...
			if(writeToDatabaseEnabled == true) {	
				...
				if(CONTROLLER_REGISTRY.getSourceParams(CONTROLLER_ID).getDrawOnSourceEnabled() == true) {
					if(VIDEO_CREATION_DATE_MODE == MetadataRuntimeParams.DATE_CREATION_MODE_REAL_ONLY) {
						startBlobStoreThread(currentCopy, blobExtractor.getBlobs(), VIDEO_CREATION_DATE, minimumMass);
					}
					else if(VIDEO_CREATION_DATE_MODE == MetadataRuntimeParams.DATE_CREATION_MODE_REAL_OR_INSTANT) {
						if(VIDEO_CREATION_DATE != null) {
							startBlobStoreThread(currentCopy, blobExtractor.getBlobs(), VIDEO_CREATION_DATE, minimumMass);
						}
						else {
					...
					else { //if(VIDEO_CREATION_DATE_MODE == MetadataRuntimeParams.DATE_CREATION_MODE_INSTANT_ONLY) {
						startBlobStoreThread(currentCopy, blobExtractor.getBlobs(), Utils.getTimeStamp(), minimumMass);
					}
				}
				else {
