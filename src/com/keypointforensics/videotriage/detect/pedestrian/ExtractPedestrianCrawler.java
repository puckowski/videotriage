package com.keypointforensics.videotriage.detect.pedestrian;
...
public class ExtractPedestrianCrawler extends ExtractDetectionModule {

	/*
	 * Author: Daniel Puckowski
	 */
						
	public ExtractPedestrianCrawler(final GuiMain guiMain, final String databaseName, final String extractDirectory) {		
	...
	protected void createDetectionDatabaseFolderIfNecessary() {
		...
		File databaseFolder = new File(FileUtils.PEDESTRIANS_DIRECTORY + databaseNameShort);
	...
	public void performDatabaseCrawlAction() {
		...
		if(mExhaustiveSearch == false) {
			searchProgressBundle = mHaarDetector.performStandardSearch(CascadeUtils.HAAR_CASCADE_PEDESTRIAN_DEFAULT, searchProgressBundle, allCapturePaths);
		} else {
			searchProgressBundle = mHaarDetector.performExhaustiveSearch(CascadeUtils.getCompletePedestrianHaarCascadeList(), searchProgressBundle, allCapturePaths);
		}
