...
public class HaarCascade {
	...
	private class Feature {
		...
		public Feature(int _id, Tree _tree, float _threshold, float _left_val, int _left_node, float _right_val,
				int _right_node) {
			id = _id;
			tree = _tree;
	...
	/**
	 * The list of classifiers that the test image should pass to be considered as
	 * an image.
	 */
	int[] stageIds;
	 ...
	 * 
	 * @param filename
	 *            The XML file (generated by OpenCV) describing the HaarCascade.
	...
	/**
	 * Detector constructor. Builds, from a XML document (i.e. the result of parsing
	 * an XML file, the corresponding Haar cascade.
	 * 
	 * @param document
	 *            The XML document (parsing of file generated by OpenCV) describing
	 *            the Haar cascade.
	 * 
	...
	public HaarCascade(Document document) {
			...
			for (Element treeElement : new Itr(treesElement)) {
				...
				for (Element featureElement : new Itr(treeElement)) {
					...
					for (Element rectElement : new Itr(rectsElement)) {
						...
						Rect rect = new Rect(rect_ids++, Integer.parseInt(rectValues[0]),
								Integer.parseInt(rectValues[1]), Integer.parseInt(rectValues[2]),
								Integer.parseInt(rectValues[3]), Float.parseFloat(rectValues[4]));
		...
		// now we take the above generated data structure apart and create a
		// data parallel friendly form.
	...
	boolean pass(int stageId, int[] grayImage, int[] squares, int width, int height, int i, int j, float scale) {
		float sum = 0;
		for (int treeId = stage_startEnd[stageId * STAGE_INTS + 0]; treeId <= stage_startEnd[stageId * STAGE_INTS
				+ 1]; treeId++) {
			...
			int totalXIndex1, totalXIndex2, totalXIndex3, totalXIndex4;
			int totalX2Index1, totalX2Index2, totalX2Index3, totalX2Index4;
			
			int r, w, h, total_x, total_x2, rect_sum, rectId, x1, y1, x2, y2, rx1, ry1, rx2, ry2, leftNodeId, rightNodeId;
			double inv_area, moy, vnorm, rect_sum2;
			float weight;
			
			while (!done) {
				// System.out.println("feature id "+featureId);

				w = (int) (scale * cascadeWidth);
				h = (int) (scale * cascadeHeight);
				inv_area = 1. / (w * h);
				// System.out.println("w2 : "+w2);
				
				totalXIndex1 = i + w + (j + h) * width;
				totalXIndex2 = i + (j) * width;
				totalXIndex3 = i + (j + h) * width;
				totalXIndex4 = i + w + (j) * width;
				if(totalXIndex1 > grayImage.length ||
					totalXIndex2 > grayImage.length ||
					totalXIndex3 > grayImage.length ||
					totalXIndex4 > grayImage.length) {
					return sum > stage_thresh[stageId * STAGE_FLOATS + 0];//false;
					//continue;
				}
				total_x = grayImage[totalXIndex1] + grayImage[totalXIndex2]
						- grayImage[totalXIndex3] - grayImage[totalXIndex4];
				
				totalX2Index1 = i + w + (j + h) * width;
				totalX2Index2 = i + (j) * width;
				totalX2Index3 = i + (j + h) * width;
				totalX2Index4 = i + w + (j) * width;
				if(totalX2Index1 > squares.length ||
					totalX2Index2 > squares.length ||
					totalX2Index3 > squares.length ||
					totalX2Index4 > squares.length) {
					return sum > stage_thresh[stageId * STAGE_FLOATS + 0];//false;
					//continue;
				}
				total_x2 = squares[totalX2Index1] + squares[totalX2Index2]
						- squares[totalX2Index3] - squares[totalX2Index4];
				...
				rect_sum = 0;
				for (r = 0; r < 3; r++) {
					rectId = feature_r1r2r3LnRn[featureId * FEATURE_INTS + r];
					if (rectId != -1) {
						...
						weight = rect_w[rectId * RECT_FLOATS + 0];
						rx1 = i + (int) (scale * x1);
						rx2 = i + (int) (scale * (x1 + y1));
						ry1 = j + (int) (scale * x2);
						ry2 = j + (int) (scale * (x2 + y2));
				...
				// System.out.println(rect_sum2+" "+ Feature.LvRvThres[featureId
				// * Feature.FLOATS + 2]*vnorm);

				if (rect_sum2 < feature_LvRvThres[featureId * FEATURE_FLOATS + 2] * vnorm) {

					leftNodeId = feature_r1r2r3LnRn[featureId * FEATURE_INTS + 3];
					if (leftNodeId == -1) {
					...
					rightNodeId = feature_r1r2r3LnRn[featureId * FEATURE_INTS + 4];
					if (rightNodeId == -1) {
