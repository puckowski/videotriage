...
public abstract class DetectorAlt {
	 ...
	 * which regions should be considered as searched objects. Please see Wikipedia
	 * for a description of the algorithm.
	 * 
	 * @param file
	 *            The image file to scan.
	 * @param baseScale
	 *            The initial ratio between the window size and the Haar classifier
	 *            size (default 2).
	 * @param scale_inc
	 *            The scale increment of the window size, at each step (default
	 *            1.25).
	 * @param increment
	 *            The shift of the window at each sub-step, in terms of percentage
	 *            of the window size.
	...
	public List<Rectangle> getFeatures(BufferedImage image) {
		...
		final float maxScale = (Math.min((width + 0.f) / haarCascade.cascadeWidth,
				(height + 0.0f) / haarCascade.cascadeHeight));
		...
		if (dataBuffer instanceof DataBufferByte) {
			...
			for (int j = 0; j < height; j++) {
				...
				for (int i = 0; i < width; i++) {
					...
					col += value;
					weightedGrayImage[(i + j * width)] = (j > 0 ? weightedGrayImage[(i + j * width) - width] : 0) + col; // NOT
																															// data
																															// parallel
																															// !
					col2 += value * value;
					weightedGrayImageSquared[(i
							+ j * width)] = (j > 0 ? weightedGrayImageSquared[(i + j * width) - width] : 0) + col2; // NOT
																													// data
																													// parallel
				}
			}
			...
			for (int i = 0; i < width; i++) {
				...
				for (int j = 0; j < height; j++) {
					...
					grayImage[j] = value;
					col += value;
					weightedGrayImage[i + j * width] = (i > 0 ? weightedGrayImage[i - 1 + j * width] : 0) + col; // NOT
																													// data
																													// parallel
																													// !
					col2 += value * value;
					weightedGrayImageSquared[i + j * width] = (i > 0 ? weightedGrayImageSquared[i - 1 + j * width] : 0)
							+ col2; // NOT data parallel
				}
			}
		}
	...
	abstract List<Rectangle> getFeatures(final int width, final int height, float maxScale,
			final int[] weightedGrayImage, final int[] weightedGrayImageSquared);
