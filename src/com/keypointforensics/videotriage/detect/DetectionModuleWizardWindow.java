...
public class DetectionModuleWizardWindow extends JFrame implements ActionListener, ItemListener, ChangeListener {
	...
	public DetectionModuleWizardWindow(final GuiMain guiMain, final String databaseName) {		
		GUI_MAIN = guiMain;
		...
		for(int i = 0; i < numberOfModulesToRun; ++i) {
			...
			switch(detectionModuleEnum) {
				case DETECTION_MODULE_FACE: {
					detectionModule = new DatabaseFaceCrawler(GUI_MAIN, DATABASE_NAME);
							
					break;
				}
				case DETECTION_MODULE_LICENSE_PLATE: {
					detectionModule = new DatabaseLicensePlateCrawler(GUI_MAIN, DATABASE_NAME);
							
					break;
				}
				case DETECTION_MODULE_PEDESTRIAN: {
					detectionModule = new DatabasePedestrianCrawler(GUI_MAIN, DATABASE_NAME);
					
					break;
				}
				case DETECTION_MODULE_CAR: {
					detectionModule = new DatabaseCarCrawler(GUI_MAIN, DATABASE_NAME);
					
					break;
				}
				case DETECTION_MODULE_EXPLICIT: {
					detectionModule = new DatabaseExplicitCrawler(GUI_MAIN, DATABASE_NAME);
		...
		if (event.getSource() instanceof JButton) {
			...
			if(button == mAddDetectionModuleButton) {
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("DetectionModWizWindow AddDetectionModule", this);
				...
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("DetectionModWizWindow PerformStart", this);
						...
						DetectionModuleWizardWindow.this.dispose();
				...
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("DetectionModWizWindow RemoveSelectedModule", this);
		...
		menuItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {	
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("DetectionModWizWindow Exit", this);
		...
		menuItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {	
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("DetectionModWizWindow OpenDoc", this);
