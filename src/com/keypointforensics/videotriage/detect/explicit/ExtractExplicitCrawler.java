package com.keypointforensics.videotriage.detect.explicit;
...
public class ExtractExplicitCrawler extends ExtractDetectionModule {

	/*
	 * Author: Daniel Puckowski
	 */
						
	public ExtractExplicitCrawler(final GuiMain guiMain, final String databaseName, final String extractDirectory) {		
	...
	protected void createDetectionDatabaseFolderIfNecessary() {
		...
		File databaseFolder = new File(FileUtils.EXPLICIT_DIRECTORY + databaseNameShort);
	...
	public void performDatabaseCrawlAction() {
		//Utils.displayMessageDialog("Notice", "Explicit image detection is currently in beta.");
		...
		if(mExhaustiveSearch == false) {
			searchProgressBundle = mHaarDetector.performStandardSearch(CascadeUtils.HAAR_CASCADE_EXPLICIT_DEFAULT, searchProgressBundle, allCapturePaths);
		} else {
			searchProgressBundle = mHaarDetector.performExhaustiveSearch(CascadeUtils.getCompleteExplicitHaarCascadeList(), searchProgressBundle, allCapturePaths);
		}
