...
public class AdvancedDeleteDataWindow extends BaseAdvancedDeleteDataWindow implements ActionListener {
	...
	public AdvancedDeleteDataWindow(final GuiMain guiMain) {
		GUI_MAIN = guiMain;
		
		WindowRegistry.getInstance().registerFrame(this, "AdvancedDeleteData");
	}
	...
	private void deleteDataAction() {
		new Thread() {
			@Override
			public void run() {
				ThreadUtils.addThreadToHandleList("AdvancedDeleteData FileExit", this);
				...
				if(mRemovalStartDate != null) {
					AdvancedDeleteDataWindow.this.dispose();
		...
		menuItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {	
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("AdvancedDeleteData OpenDoc", this);
