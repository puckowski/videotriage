package com.keypointforensics.videotriage.gui.resize;
...
public class ResizeLocalFileWizardWindow extends JFrame implements ActionListener, ItemListener {
	...
	public ResizeLocalFileWizardWindow(final JFrame parentFrame) {
		PARENT_FRAME = parentFrame;
		
		buildFrame();
		
		WindowRegistry.getInstance().registerFrame(this, "ResizeLocalFileWizard");
	}
	...
	private void buildFrame() {		
		...
		this.setTitle("Local File Resize Wizard");
		this.pack();
	...
	private void performStartAction() {
		if(mResolutionComboBox.getSelectedIndex() == -1) {
			Utils.displayMessageDialog("No Resolution", "Please select a resize video resolution from the drop-down menu.");
			
			return;
		}
		
		ResizeLocalFileWizardWindow.this.dispose();
		
		ArrayList<String> localFilesToProcess = LocalFileWizardUtils.getLocalWizardListOfFiles(mFileDropList.getFileList());

		CreateResizedVideoThread createResizedVideoThread = new CreateResizedVideoThread(PARENT_FRAME, localFilesToProcess, getSelectedPreviewWidth(), getSelectedPreviewHeight());
		createResizedVideoThread.start();
		
		try {
			createResizedVideoThread.join();
		...
		if (event.getSource() instanceof JButton) {
			...
			if(button == mSelectFilesButton) {
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("ResizeLocFileWizWindow SelectFiles", this);
				...
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("ResizeLocFileWizWindow PerformStart", this);
				...
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("ResizeLocFileWizWindow RemoveSelected", this);
		...
		menuItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {	
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("ResizeLocFileWizWindow Exit", this);
		...
		menuItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {	
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("ResizeLocFileWizWindow Select", this);
		...
		menuItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {	
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("ResizeLocFileWizWindow Start", this);
		...
		menuItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {	
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("ResizeLocFileWizWindow OpenDoc", this);
		...
		menuItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {	
				new Thread() {
					@Override
					public void run() {
						ThreadUtils.addThreadToHandleList("ResizeLocFileWizWindow About", this);
