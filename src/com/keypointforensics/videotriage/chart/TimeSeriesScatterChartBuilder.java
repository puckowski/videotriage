...
public class TimeSeriesScatterChartBuilder extends VideoTriageChartBuilder {
	...
	private final String JAVASCRIPT_FILENAME_SHORT = "time_scatter.js";
	private final String UNIQUE_DIV_NAME           = "TimeScatter";
	...
	public TimeSeriesScatterChartBuilder(final String reportRoot, final String reportPageHeader) {
	...
	private void prepareNewTimeSeriesChart() {
		mChartBuilder.append("var layout={margin:{r:180,t:40,b:140,l:100},title:'Time Series',xaxis:{title:'Date',tickangle:45},yaxis:{title:'Number of Keypoints'}};var data=[{type:'scatter',mode:'markers',x:[");
	}
	...
	private void appendMarkerSize(final int value) {
		if(mFirstTimeValue == false) {
			mChartBuilder.append(",");
		} else {
			mFirstTimeValue = false;
	...
	public void build(boolean isReportPaginated) {	
		...
		for(int i = 0; i < mEventTimeline.size(); ++i) {
			appendMarkerSize(mEventMap.get(mEventTimeline.get(i)));
		}
		mChartBuilder.append("]}");
