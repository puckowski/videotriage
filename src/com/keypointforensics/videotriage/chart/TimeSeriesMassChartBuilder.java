...
public class TimeSeriesMassChartBuilder extends VideoTriageChartBuilder {
	...
	private final String JAVASCRIPT_FILENAME_SHORT = "time_mass.js";
	private final String UNIQUE_DIV_NAME           = "TimeMass";
	...
	public TimeSeriesMassChartBuilder(final String reportRoot, final String reportPageHeader) {
		super(reportRoot, reportPageHeader);
		
		mEventTimeline = new ArrayList<String>();
		mEventMap      = new HashMap<String, Long>();
	...
	private void prepareNewTimeSeriesChart() {
		mChartBuilder.append("var layout={margin:{r:180,t:40,b:140,l:100},title:'Time Series',xaxis:{title:'Date',tickangle:45},yaxis:{title:'Mass of Keypoints'}};var data=[{type:'scatter',x:[");
	}
	...
	public void addEvent(final String eventTime, final long mass) {		
		...
		if(mEventMap.containsKey(correctedEventTime) == false) {
			mEventMap.put(correctedEventTime, mass);
		} else {
			mEventMap.replace(correctedEventTime, mEventMap.get(correctedEventTime) + mass);
		}
	}
	...
	private void appendTimeValue(final long value) {
